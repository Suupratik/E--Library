...
const UserApi = {
  borrowBook: async (isbn, userId) => {
    const res = await fetch(`${API_BASE_URL}/v1/user/borrow`, {
      method: "POST",
  ...
  returnBook: async (isbn, userId) => {
    const res = await fetch(`${API_BASE_URL}/v1/user/return`, {
      method: "POST",
      body: JSON.stringify({ isbn, userId }),
      headers: { "Content-Type": "application/json" },
    });
  ...
  getBorrowBook: async () => {
    const res = await fetch(`${API_BASE_URL}/v1/user/borrowed-books`, {
      method: "GET",
    });
    return res.json();
  },
